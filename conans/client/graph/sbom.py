...
def generate_sbom(conanfile):
    ...
    packages_to_check = [conanfile]
    packages = []
    while packages_to_check:
        dependency = packages_to_check.pop()
        packages.append(
            {
        ...
        packages_to_check += [dependency for require, dependency in dependency.dependencies.items()]
    files = []
    ...
    path = "./spdx/"
    file_name = f"{conanfile.ref}-SPDX.json".replace("/", "-")
    try:
